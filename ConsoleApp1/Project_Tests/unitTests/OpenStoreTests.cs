using ConsoleApp1.domainLayer.Business_Layer;
using NUnit.Framework;
namespace Project_tests.unitTests
{
    public class OpenStoreTests : ATProject
    {
        private User member;
        [SetUp]
        public void Setup()
        {
            User member = new User("admin","admin");
            member.Login();
        }

        [Test]
        public void Happy()
        {
            string policy = "policy";
            string name = "test";
            Store store = OpenStore(member, policy, name);
            Assert.NotNull(store);
            Assert.True(store.SellingPolicy.Equals(policy));
            Assert.True(store.Owner.Equals(member));
            Assert.True(store.Name.Equals(name));
            
            // remove the store

        }

        [Test]
        public void Sad()
        {
            string policy = "policy2";
            string name = "test2";
            Store store = OpenStore(member, policy, name);
            Assert.NotNull(store);
            Assert.False(!store.SellingPolicy.Equals(policy));
            Assert.False(!store.Owner.Equals(member));
            Assert.False(!store.Name.Equals(name));
        }

        [Test]
        public void Bad()
        {
            string policy = "policy";
            string name = "test";
            Store store = OpenStore(member, policy, name);
            Assert.NotNull(store);
            Assert.True(store.SellingPolicy.Equals(policy));
            Assert.True(store.Owner.Equals(member));
            Assert.True(store.Name.Equals(name));

            User guest = new User("guest","guest");
            Assert.NotNull(GetStoreInfo(guest, name));
        }

        [Test]

        public void ShouldFail()
        {
            // not a member trying to open a store
            // trying to open a store with the same name and/or the same manager
        }
    }
}